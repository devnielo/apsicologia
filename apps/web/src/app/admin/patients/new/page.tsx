...
// Schema Zod alineado exactamente con el modelo Patient del backend
const patientSchema = z.object({
  personalInfo: z.object({
    ...
    occupation: z.string().max(100, 'La ocupación no puede exceder 100 caracteres').optional(),
    employer: z.string().max(100, 'El empleador no puede exceder 100 caracteres').optional(),
  ...
  contactInfo: z.object({
    ...
    preferredContactMethod: z.enum(['email', 'phone', 'sms', 'whatsapp'], {
      required_error: 'Seleccione un método de contacto preferido'
    }),
    address: z.object({
      street: z.string({
  ...
  emergencyContact: z.object({
    ...
    relationship: z.string({
      ...
      .min(1, 'La relación no puede estar vacía')
      .max(50, 'La relación no puede exceder 50 caracteres')
      .trim(),
    phone: z.string({
      required_error: 'El teléfono del contacto de emergencia es requerido'
    })
      .min(9, 'El teléfono debe tener al menos 9 dígitos')
      .trim(),
    email: z.string().email('Formato de email inválido').trim().optional().or(z.literal('')),
  }),
  ...
  gdprConsent: z.object({
    dataProcessing: z.object({
      consented: z.boolean({
        required_error: 'Debe aceptar el procesamiento de datos'
      }),
      consentDate: z.date({
        required_error: 'Fecha de consentimiento requerida'
      }),
      consentMethod: z.enum(['verbal', 'written', 'digital'], {
        required_error: 'Método de consentimiento requerido'
      }),
    ...
    dataSharing: z.object({
      healthcareProfessionals: z.boolean().default(true),
      insuranceProviders: z.boolean().default(false),
      emergencyContacts: z.boolean().default(true),
      researchPurposes: z.boolean().default(false),
      consentDate: z.date({
        required_error: 'Fecha de consentimiento para compartir datos requerida'
      }),
    }),
  }),
...
export default function PatientDetailsPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      // El error ya se maneja en las mutaciones con toast
    }
  };
  ...
  // Función para obtener errores de servidor específicos
  const getFieldError = (fieldPath: string) => {
    const serverError = createMutation.error || updateMutation.error;
    if (serverError?.response?.data?.errors) {
      return serverError.response.data.errors[fieldPath];
    }
    return null;
  };
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {isNewPatient ? 'Creando...' : 'Guardando...'}
                  </>
                ) : (
                  <>
        ...
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            {createMutation.error?.response?.data?.message || 
             ...
             'Ha ocurrido un error. Por favor, revise los campos e intente nuevamente.'}
          </AlertDescription>
        </Alert>
      )}
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            {isEditing ? (
                              ...
                              <div className="px-3 py-2 bg-muted rounded-md">
                                {GENDER_OPTIONS.find(option => option.value === field.value)?.label || 'No especificado'}
                              </div>
                            )}
                          </FormControl>
                          <FormMessage />
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            {isEditing ? (
                              ...
                              <div className="px-3 py-2 bg-muted rounded-md">
                                {OCCUPATION_OPTIONS.find(option => option.value === field.value)?.label || field.value || 'No especificada'}
                              </div>
                            )}
                          </FormControl>
                          <FormMessage />
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          {isEditing ? (
                            ...
                            <div className="px-3 py-2 bg-muted rounded-md">
                              {MARITAL_STATUS_OPTIONS.find(option => option.value === field.value)?.label || 'No especificado'}
                            </div>
                          )}
                        </FormControl>
                        <FormMessage />
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          {isEditing ? (
                            <Select onValueChange={field.onChange} value={field.value} disabled={isSubmitting}>
                              <SelectTrigger>
                                <SelectValue placeholder="Seleccione método preferido" />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            <div className="px-3 py-2 bg-muted rounded-md">
                              {field.value === 'email' ? 'Email' :
                               field.value === 'phone' ? 'Teléfono' :
                               field.value === 'sms' ? 'SMS' :
                               field.value === 'whatsapp' ? 'WhatsApp' : 'No especificado'}
                            </div>
                          )}
                        </FormControl>
                        <FormMessage />
                        {getFieldError('contactInfo.preferredContactMethod') && (
                          <p className="text-sm text-destructive mt-1">
                            {getFieldError('contactInfo.preferredContactMethod')}
                          </p>
                        )}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        {isEditing ? (
                          <Input 
                            {...field} 
                            placeholder="Ej: Calle de la Paz, 123, 2º A"
                            disabled={isSubmitting}
                          />
                        ) : (
                          <div className="px-3 py-2 bg-muted rounded-md">{field.value || 'No especificada'}</div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          {isEditing ? (
                            form.watch('contactInfo.address.country') === 'España' ? (
                              <Select onValueChange={field.onChange} value={field.value} disabled={isSubmitting}>
                                <SelectTrigger>
                              ...
                              <Input 
                                {...field} 
                                value={field.value || ''} 
                                placeholder="Ej: Madrid, Estado, Región"
                                disabled={isSubmitting}
                              />
                            )
                          ) : (
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormMessage />
                        {getFieldError('contactInfo.address.country') && (
                          <p className="text-sm text-destructive mt-1">
                            {getFieldError('contactInfo.address.country')}
                          </p>
                        )}
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          {isEditing ? (
                            ...
                            <div className="px-3 py-2 bg-muted rounded-md">
                              {EMERGENCY_CONTACT_RELATIONSHIP_OPTIONS.find(option => option.value === field.value)?.label || field.value || 'No especificada'}
                            </div>
                          )}
                        </FormControl>
                        <FormMessage />
