...
export default function LoginPageClient() {
  ...
  // Check if user is already logged in - now safe since this only runs on client
  useEffect(() => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      // Wait for context to update localStorage
      setTimeout(() => {
      ...
      }, 100); // Small delay to ensure localStorage updates
    } catch (error: any) {
